...
process_synthpool_biofrag_sRNAnalysis <- function(biofragmenta.outdir,
  ...
  message(paste0(Sys.time()-tstart, " -- ", biofragmenta.outdir, "-- START: ", "sRNAnalysis Import: ", "Sample Info", "File.Base.ID"))
  sample.info.dt <- fread(sample.info.fl)
  ...
  processed.read.fls <- dir(path = sRNA.out.basedir, pattern = "*_Processed.fa", full.names = TRUE, recursive = TRUE)
    ...
    feature.count.fls <- feature.count.fls[sub("_L00[1-4]_R1_001_.*", "", basename(feature.count.fls))%in%sample.info.dt$File.Base.ID]
    unaligned.count.fls <- unaligned.count.fls[sub("_L00[1-4]_R1_001_.*", "", basename(unaligned.count.fls))%in%sample.info.dt$File.Base.ID]
    processed.read.fls <- processed.read.fls[sub("_L00[1-4]_R1_001_.*", "", basename(processed.read.fls))%in%sample.info.dt$File.Base.ID]
  ...
  merge.cols <- intersect(colnames(sample.info.dt), colnames(all.feature.dt))
  all.freature.dt.annot <- merge(sample.info.dt, all.feature.dt, by=merge.cols)
  ...
  message(paste0(Sys.time()-tstart, " -- ", biofragmenta.outdir, "-- DONE ----"))
  all.processed[sample.info.dt, Input.ReadCount:=i.Input.ReadCount, on="File.Base.ID"]
  read.count.summary <- merge(all.processed, all.unaligned, by=c("File.Base.ID", "Sample.ID", "IndexNum"))
  ...
  dt.list <- list(input.files=list(profile.counts=feature.count.fls,
                  ...
                  read.count.summary=read.count.summary2[sample.info.dt, library.method:=i.library.method, on="File.Base.ID"],
                  annot.summary=all.freature.dt.annot.summary[sample.info.dt, library.method:=i.library.method, on=intersect(colnames(all.freature.dt.annot.summary), colnames(sample.info.dt))],
                  annot.summary.mm=all.freature.dt.annot.summary.mm[sample.info.dt, library.method:=i.library.method, on=intersect(colnames(all.freature.dt.annot.summary.mm), colnames(sample.info.dt))],
                  feature.count.dt=feat.count.dt[sample.info.dt, library.method:=i.library.method, on=intersect(colnames(feat.count.dt), colnames(sample.info.dt))],
                  all.feature.dt=all.freature.dt.annot.filt2[sample.info.dt, library.method:=i.library.method, on=intersect(colnames(all.freature.dt.annot.filt2), colnames(sample.info.dt))]
  )
